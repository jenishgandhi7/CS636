...
//	}
	 ...
	 * Get cart info
	 *
	 * @return the cart's content API design: OK to use Set<CartItem> here for
	 *         return value, but then need getProductById in service API because
	 *         CartItem only has the product id, not the whole Product
	 */
	 ...
	 * @param quantity
	 */
	public void addItemtoCart(long prod_Id, Cart cart, int quantity) {
	...
	/**
	 * Change the quantity of one item. If quantity <= 0 then delete this item. API
	 * design: first argument can be productId here (good choice actually)
	 *
	 * @param prod
	 * @param cart
	 * @param quantity
	...
	/**
	 * Remove a product item from the cart API design: first argument can be
	 * productId here (good choice actually)
	 *
	 * @param prod
	 * @param cart
	 */
	...
	/**
	 * Get user info by given email address Note: here we assume the presentation
	 * layer received the user email string (the argument here) from the user.
	 *
	 * @param email
	 ...
	 * @param quantity
	 */
	public void del_item_Cart(long productId, Cart cart) {
	...
	public Product getProduct(long productId) throws ServiceException {
		...
		try {
			connection = db.startTransaction();
			Product prd = prodDb.findProductByPID(connection, productId);
			if (prd == null)
				return null;
	 ...
	 *
	 * @param userId id of user who downloaded the track
	 * @param track  the track which was downloaded
	 * @throws ServiceException
	 */
