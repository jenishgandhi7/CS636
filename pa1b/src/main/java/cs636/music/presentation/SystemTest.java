...
public class SystemTest {
	...
	public SystemTest(String inFile, String dbUrl, String usr, String passwd) throws Exception {
		this.inFile = inFile;
	...
	public void run() throws IOException, ServiceException {
		...
		while ((command = getNextCommand(in)) != null) {
			...
			{
				Set<Product> cdList = userService.getListofProduct();
				if (cdList != null)
				...
				UserData user = userService.getUserDetails(usr);
				if (user == null)
					System.out.println("\nNo such user" + usr + "\n");
				else
				...
				userService.NewUser_register(userInfo[1], userInfo[2], userInfo[3]);
				...
				int track_num = Integer.parseInt(params[3]);
				...
				if (product == null)
					...
					UserData user = userService.getUserDetails(userEmail);
					if (user == null)
						System.out.println("\nNo such user\n");
					else {
						Track match_track = null;
						for (Track track : tracks) {
							if (track.getTrackNumber() == track_num) {
								match_track = track;
							}
						}
						if (match_track != null) {
							System.out.println("Recording download for user " + userEmail);
							userService.Download_Record(user.getId(), match_track);
						} else {
							System.out.println("\nNo such track\n");
						}
					}
				...
				String params[] = getTokens(command);
				UserData user = userService.getUserDetails(params[1]);
				if (user == null)
					System.out.println("\n No such user\n");
				else {
				...
				String params[] = getTokens(command);
				Product product = userService.getProductByPro_Code(params[1]);
				if (product == null)
					System.out.println("\nNo such product\n");
				else {
					userService.addItemIntoCart(product, cart, 1);
					
					System.out.println(params[1]+" Product Added to Cart.." );
				}
